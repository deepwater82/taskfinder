...
#
# classifier.py
#  - Make dictionary (binary) file, spit it out
#
######################################################
...
def main():
    ...
    if len(args)!=2:
        print '\n###############################################################'
        print 'usage: python classifier.py <input directory> <output directory>\n'
    ...
    # Get first and end date
    dates_tupl = sort(dev_lst)
    
    # Make bins
    make_bin(dates_tupl)

    # Process
    make_dictionary_file(:e 
    # Start process
    for dev in dev_lst:
